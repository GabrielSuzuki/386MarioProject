...
class Coin(pg.sprite.Sprite):
    ...
    def __init__(self, x, y, score_group, collect=False, mario= None):
    ...
    def setup_frames(self):
        """Create frame list"""
        self.frames.append(
            self.get_image(0, 96, 16, 16))
        self.frames.append(
            self.get_image(16, 96, 16, 16))
        self.frames.append(
            self.get_image(32, 96, 16, 16))
        self.frames.append(
            self.get_image(48, 96, 16, 16))
    ...
    def spinning(self):
        ...
        if self.collect == True:
            collide = self.rect.collidepoint(self.mario.rect.x,self.mario.rect.y)
            if collide == True:
                self.kill()
